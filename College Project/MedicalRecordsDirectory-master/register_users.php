...
if (isset($_POST['submit'])) {
    ...
    $past_surgeries = mysqli_real_escape_string($db, $_POST['past_surgeries']);
    $ongoing_medications = mysqli_real_escape_string($db, $_POST['ongoing_medications']);
    $email = mysqli_real_escape_string($db, $_POST['email']);
    $password = mysqli_real_escape_string($db, $_POST['password']);
    $password = md5($password);
    $query = "INSERT INTO users (`user_id`, `name`, `dob`, `gender`, `state`, `phone`, `bloodgroup`, `allergies`, `pastsurgeries`, `ongoingmedications`,`email`, `password`) 
  			  VALUES('$user_id' , '$name', '$dob', '$gender', '$state', '$phone', '$bg', '$allergies', '$past_surgeries', '$ongoing_medications','$email','$password')";

    mysqli_query($db, $query);

    // $query_table = "CREATE TABLE `$user_id` ( `sl` INT NOT NULL AUTO_INCREMENT , `symptoms` VARCHAR(255) NOT NULL , `diagnosis` VARCHAR(255) NOT NULL , `date` DATE NOT NULL , `dr` VARCHAR(255) NOT NULL , `trtmnt` VARCHAR(500) NOT NULL , `medicines` VARCHAR(200) NOT NULL , PRIMARY KEY (`sl`))";
    // mysqli_query($db, $query_table);

    // mkdir($user_id, 0777);

    // $user_id_generated = "Registration Successful.<br>Your Medical Legacy Number is <b>" . $user_id . "</b>.<br> Please note it down for future use. Proceed to <a href='userLogin.php'>Login</a>";
}    
?>
<html>
    <head>
        ...
        <div>
            <div>
                <div class="col-lg-offset-1 col-lg-10 box" style="vertical-align: middle; padding: 20px">
                    <form action="register_users.php" class="content-section" method="POST">
                        <h1 style="font-size: 50px; ">REGISTERATION FOR USER</h1>
<?php
...
?>
                        ...
                        <div class="form-input-group">
                            <div class="row">
                                <label style="float: left" class="col-lg-2">Blood Group</label>
                                <select name="bg" class="input-box col-lg-4">
                                ...
                                <label style="text-align: center; vertical-align: middle;" class="col-lg-2">Allergies</label><input name="allergies" type="text" class="input-box col-lg-4">
                            </div>
                        </div>
                        <div class="row form-input-group">
                            <label style="text-align: left" class="col-lg-2">Past Surgeries</label><input name="past_surgeries" type="text" class="input-box col-lg-4">
                            <label style="text-align: center" class="col-lg-2">Ongoing Medications</label><input name="ongoing_medications" type="text" class="input-box col-lg-4">
                        </div>
                        <div class="row form-input-group">
